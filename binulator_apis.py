...
def ocenmock_api(data_file,verr,Nbin,axis):
    #Read in the data:
    ...
    #Set the project to be along x,y and/or z:
    ...
    #Calculate flattening along this projection:
    #Follows: https://arxiv.org/pdf/1511.01097.pdf
    inertia = np.zeros((3, 3))
    r = np.sqrt(data[:,0]**2.0+data[:,1]**2.0+data[:,2]**2.0)
    rcut = np.array([0.001,0.005,0.01,0.05,0.1,0.5,1.0])
    for i in range(len(rcut)):
        x = data[:,0][r<rcut[i]]
        y = data[:,1][r<rcut[i]]
        inertia[0, 0] = np.sum(x*x)
        inertia[1, 1] = np.sum(y*y)
        inertia[0, 1] = np.sum(x*y)
        inertia[1, 0] = inertia[0, 1]
        eigenvalues, eigenvectors = np.linalg.eig(inertia)
        ellipticity = 1.0 - np.abs(eigenvalues[0]-eigenvalues[1])/\
                      (eigenvalues[0]+eigenvalues[1])    
        print('Rcut, Ellipticity:', rcut[i], ellipticity)
    ...
    plt.xlim([-spherer_in,spherer_in])
    plt.ylim([-spherer_in,spherer_in])
    ...
    print('Total effective no. of tracers (photometric):', np.sum(ms))
    print('Min/max stellar mass:', np.min(data[:,6]), np.max(data[:,6]))
